...
try {
    ...
    // 1. Actualizar receta
    $stmt = $pdo->prepare(
        'UPDATE recetas SET titulo = ?, pasos = ?, tiempo_min = ? WHERE id = ?'
    );
    $stmt->execute([$titulo, $pasos, $tiempo_min, $receta_id]);

    // 2. Reinsertar ingredientes
    $pdo->prepare('DELETE FROM ingredientes WHERE receta_id = ?')->execute([$receta_id]);
    $stmtIng = $pdo->prepare(
    ...
    // 3. ✅ Reinsertar etiquetas (borrar las anteriores y poner las nuevas)
    $pdo->prepare('DELETE FROM receta_etiquetas WHERE receta_id = ?')->execute([$receta_id]);
    if (!empty($etiquetas)) {
        $stmtTag = $pdo->prepare(
