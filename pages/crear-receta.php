...
<body>
        ...
        <div class="form-card">
            ...
            <form id="formReceta" novalidate>
                ...
                <div id="ingredientesContainer">
                    <!-- Las filas de ingredientes se insertan aquí -->
                </div>
    ...
    <script>
      ...
      <input type="text"   name="ingredientes[${idx}][nombre]"   class="form-control"
             placeholder="Ingrediente" value="${nombre}" required>
      <input type="text"   name="ingredientes[${idx}][cantidad]" class="form-control"
             placeholder="Cantidad" value="${cantidad}">
      <button type="button" class="btn-remove-ing" onclick="eliminarIngrediente('ing-${idx}')">×</button>
    `;
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (!tiempo_min) {
                showError('El tiempo de preparación es obligatorio.');
                return;
            }
            if (!pasos) {
                showError('Los pasos de preparación son obligatorios.');
                return;
            }
            if (!tieneIng) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    setTimeout(() => window.location.href = `detalle.php?id=${data.receta_id}`, 1200);
                } else {
